...
build_docx.py
=============
Generates "RideEase_Architecture_Implementation_Updated.docx" — a complete
system architecture & implementation report for the RideEase academic
project — using python-docx (pure Python, no Node.js).

Run:
	python3 build_docx.py

Requires: python-docx and Pillow.
Expects the four diagram PNGs generated by make_diagrams.py in the same directory.
"""
...
OUT_PATH = os.path.join(BASE_DIR, "RideEase_Architecture_Implementation_Updated.docx")

NAVY = RGBColor(0x1B, 0x2A, 0x4A)
ORANGE = RGBColor(0xE0, 0x7A, 0x1F)
BLUE = RGBColor(0x2D, 0x5D, 0xA8)
GREY = RGBColor(0x6B, 0x72, 0x80)
TEXT = RGBColor(0x22, 0x22, 0x22)
WHITE = RGBColor(0xFF, 0xFF, 0xFF)
LIGHT_GREY_HEX = "F4F5F7"
NAVY_HEX = "1B2A4A"
ORANGE_HEX = "E07A1F"
BORDER_HEX = "D9DCE1"
...
def _set_heading_style(name, size, color, bold=True, space_before=14, space_after=6):
...
_set_heading_style("Heading 1", 18, NAVY, space_before=20, space_after=8)
_set_heading_style("Heading 2", 13, BLUE, space_before=14, space_after=6)
_set_heading_style("Heading 3", 11.5, RGBColor(0x44, 0x44, 0x44), space_before=10, space_after=5)
...
def set_cell_borders(cell, hex_color=BORDER_HEX, size=4):
	tcPr = cell._tc.get_or_add_tcPr()
...
def add_bottom_border(paragraph, color=ORANGE_HEX, size=8, space=6):
...
def code_block(lines):
	for index, line in enumerate(lines):
		...
		paragraph.paragraph_format.space_after = Pt(0) if index < len(lines) - 1 else Pt(10)
		properties = paragraph._p.get_or_add_pPr()
		shading = OxmlElement("w:shd")
		shading.set(qn("w:val"), "clear")
		shading.set(qn("w:color"), "auto")
		shading.set(qn("w:fill"), LIGHT_GREY_HEX)
...
def figure(path, max_width_in=6.3):
	with Image.open(path):
		pass
	paragraph = doc.add_paragraph()
	paragraph.alignment = WD_ALIGN_PARAGRAPH.CENTER
	paragraph.add_run().add_picture(path, width=Cm(max_width_in * 2.54))


def data_table(header_row, rows, col_widths_cm):
	table = doc.add_table(rows=1, cols=len(header_row))
	table.autofit = False
	table.alignment = WD_ALIGN_PARAGRAPH.LEFT
	header_cells = table.rows[0].cells
	set_row_cant_split(table.rows[0])
	for index, text in enumerate(header_row):
		...
		cell.width = Cm(col_widths_cm[index])
	for row_index, row in enumerate(rows):
		cells = table.add_row().cells
		set_row_cant_split(table.rows[row_index + 1])
		background = LIGHT_GREY_HEX if row_index % 2 == 1 else None
		for index, text in enumerate(row):
			...
			run.font.color.rgb = TEXT
			if background:
				set_cell_shading(cell, background)
			set_cell_borders(cell)
			cell.vertical_alignment = WD_ALIGN_VERTICAL.CENTER
			cell.width = Cm(col_widths_cm[index])
...
def add_header_footer():
	for section in doc.sections:
		...
		header_paragraph.paragraph_format.tab_stops.add_tab_stop(Cm(17.0), WD_TAB_ALIGNMENT.RIGHT)
		for text in ("RideEase - System Architecture & Implementation Report", "\tAcademic Project"):
			run = header_paragraph.add_run(text)
			run.font.size = Pt(8)
			run.font.color.rgb = GREY
		add_bottom_border(header_paragraph, color=ORANGE_HEX, size=6, space=4)
		footer = section.footer
		footer_paragraph = footer.paragraphs[0]
		footer_paragraph.text = ""
		footer_paragraph.paragraph_format.tab_stops.add_tab_stop(Cm(17.0), WD_TAB_ALIGNMENT.RIGHT)
		run = footer_paragraph.add_run("Generated with Python (python-docx)\t")
		run.font.size = Pt(8)
		run.font.color.rgb = GREY
		field_run = footer_paragraph.add_run()
		field_run.font.size = Pt(8)
		field_run.font.color.rgb = GREY
